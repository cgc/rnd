...
Scenario: Intersecting a scaled shape with a ray
  ...
  When set_transform(s, scaling(2, 2, 2))
    ...
    And saved_ray ← unpack_saved_ray(xs)
  Then saved_ray.origin = point(0, 0, -2.5)
    And saved_ray.direction = vector(0, 0, 0.5)
...
Scenario: Intersecting a translated shape with a ray
  ...
  When set_transform(s, translation(5, 0, 0))
    And xs ← intersect(s, r)
    And saved_ray ← unpack_saved_ray(xs)
  Then saved_ray.origin = point(-5, 0, -5)
    And saved_ray.direction = vector(0, 0, 1)
...
Scenario: Converting a point from world to object space
  Given g1 ← group()
    ...
    And add_child(g2, s)
    And add_child(g1, g2)
    And s ← ref(g1.children()[0].children()[0])
